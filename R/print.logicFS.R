print.logicFS <- function (x, topX = 5, show.prop = TRUE, coded = FALSE, digits = 2, ...) 
  {
    ...
    if (x$type %in% c(1, 3)) 
      cat("Based On:", ifelse(x$useN, "Number", "Proportion"), "of", "OOB Observation", "\n")
    if (!is.null(x$mu)) 
    ...
    out <- data.frame(Importance = vim, Proportion = if (!is.null(x$prop)) 
      x$prop[order(x$vim, decreasing = TRUE, na.last = NA)]
      else NA, Expression = names.vim)
    if (!is.null(x$name)) 
      names(out)[3] <- x$name
